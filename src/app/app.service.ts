...
private login (res: Response | any) {
  if (res.status >= 200) {
    let errMsg = res.message || 'Logged in';
    console.log('teste login'); // log to console instead
    return Observable.throw(errMsg);
  }
}

private loginError (error: Response | any) {
    let errMsg = error.message || 'Error! Incorrect username, password and/or repository ID' 
    console.error(errMsg); // log to console instead
    return Observable.throw(errMsg);
  }
}